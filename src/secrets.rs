...
/// gcloud Application Default Credentials (user refresh token flow).
#[derive(Debug, Clone, PartialEq, Eq)]
pub struct GcloudAdcUser {
    pub client_id: String,
    pub client_secret: String,
    pub refresh_token: String,
}

/// Read `~/.config/gcloud/application_default_credentials.json` (or
/// `$CLOUDSDK_CONFIG/application_default_credentials.json`).
pub fn read_gcloud_adc() -> Result<GcloudAdcUser, SecretsError> {
    ...
    if typ != "authorized_user" {
        return Err(SecretsError::Decrypt(format!(
            "gcloud ADC at {}: expected type authorized_user, got {typ:?} \
             (service-account JSON needs a different import path)",
            path.display()
        )));
    }
    let client_id = v
    ...
    if client_id.is_empty() || client_secret.is_empty() || refresh_token.is_empty() {
        return Err(SecretsError::Decrypt(format!(
            "gcloud ADC at {}: missing client_id/client_secret/refresh_token",
            path.display()
        )));
    }
    Ok(GcloudAdcUser {
...
fn gcloud_adc_path() -> PathBuf {
    if let Ok(cfg) = std::env::var("CLOUDSDK_CONFIG") {
        let t = cfg.trim();
        if !t.is_empty() {
            return PathBuf::from(t).join("application_default_credentials.json");
        }
    }
    dirs::home_dir()
        .unwrap_or_else(|| PathBuf::from("."))
