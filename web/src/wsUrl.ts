 ...
 * behind Tailscale Serve (HTTPS/HTTP2 edge → Vite → honr). Session cookies are
 * host-scoped and ignore port, so on the Vite ports we dial the API port on the
 * same hostname and skip the proxy.
 *
 * Override with `VITE_HONR_WS_HOST` (host[:port], no scheme) when the API is
 * not on `:8080`.
 */
export function honrWsHost(
  loc: Pick<Location, "hostname" | "port" | "host"> = window.location,
): string {
  const override =
    typeof import.meta !== "undefined" &&
    typeof import.meta.env?.VITE_HONR_WS_HOST === "string"
      ? import.meta.env.VITE_HONR_WS_HOST.trim()
      : "";
  if (override) {
  ...
  if (loc.port === "5173" || loc.port === "4173") {
    const apiPort =
      typeof import.meta !== "undefined" &&
      typeof import.meta.env?.VITE_HONR_PORT === "string" &&
      import.meta.env.VITE_HONR_PORT.trim()
        ? import.meta.env.VITE_HONR_PORT.trim()
        : "8080";
